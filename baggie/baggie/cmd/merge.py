    ...
    with BagWriter(args.outfile, compress=args.compress) as outbag:
        ...
        for infile in infiles:
            with BagReader(infile) as inbag:
                meta = inbag.meta()
                start_time_as_nanos = meta.starting_time_as_nanos()
                start_times.append(start_time_as_nanos)
                end_times.append(start_time_as_nanos + meta.duration_as_nanos())
        t_start = Time(nanoseconds=max(start_times))
        t_end = Time(nanoseconds=min(end_times))

        for infile in infiles:
            with BagReader(infile) as inbag:
                for topic, msg, t in inbag.read_messages(start_time=t_start, end_time=t_end):
